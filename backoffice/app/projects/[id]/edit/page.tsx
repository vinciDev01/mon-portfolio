...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetchEntity<ProjectRawDto>("projects", id),
      fetchEntities<TechnologyDto>("technologies"),
    ])
      .then(([project, technologies]) => {
        setName(project.name);
        setDescription(project.description || "");
        setPhoto1Path(project.photo1Path);
        setPhoto2Path(project.photo2Path);
        ...
        setTechnologyIds(
          project.technologies?.map((pt) => pt.technologyId) || [],
        );
        setCollaborators(
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await updateEntity("projects", id, {
        name,
