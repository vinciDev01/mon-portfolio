...
export default function TechnologiesPage() {
  ...
  const [items, setItems] = useState<TechnologyDto[]>([]);
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      await deleteEntity("technologies", deleteId);
  ...
  return (
    <div>
      <PageHeader title="Technologies" addHref="/technologies/new" />
          ...
          {items.map((item) => (
            <tr key={item.id} className="border-b border-border">
              <td className="py-3 px-2">
                {item.logoPath && (
                  <img src={getFileUrl(item.logoPath)!} alt="" className="h-8 w-8 object-contain" />
                )}
              </td>
              ...
              <td className="py-3 px-2 text-right space-x-2">
                <button
                  onClick={() => router.push(`/technologies/${item.id}/edit`)}
      ...
      {items.length === 0 && <p className="text-center text-muted-foreground py-8">Aucune technologie</p>}
      <ConfirmDialog
        open={!!deleteId}
        title="Supprimer"
        message="Voulez-vous vraiment supprimer cette technologie ?"
